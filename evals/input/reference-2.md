...
Sipariş servisi, sipariş oluşturma, sorgulama, güncelleme ve iptal işlemlerini tek bir HTTP arayüzü üzerinden sunar. Tüm istekler `https://api.example.com/v1` altında toplanır, gövde formatı `application/json`'dır ve kimlik doğrulama `Authorization: Bearer <token>` header'ı ile yapılır.
...
Sipariş üzerinde kısmi güncelleme yapar. Yalnızca `note` ve `shippingAddress` alanları güncellenebilir; kalemler değiştirilemez.

### `DELETE /orders/{orderId}`

Siparişi iptal eder. Kargoya verilmiş siparişlerde iptal reddedilir.
...
| `400 Bad Request` | Doğrulama hatası; `errors` dizisinde alan bazlı detay |
...
| `422 Unprocessable Entity` | Gövde geçerli ama iş kuralı ihlal ediliyor (stok yetersiz) |
| `429 Too Many Requests` | Oran sınırı aşıldı; `Retry-After` header'ına bakın |
...
`Idempotency-Key` header'ı isteğe bağlıdır ama önerilir: aynı anahtarla tekrarlanan `POST` isteği yeni sipariş açmaz, ilk yanıtı aynen döner. Anahtar 24 saat saklanır.
