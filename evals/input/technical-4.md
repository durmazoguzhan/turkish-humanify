...
Bir index, ancak tablonun küçük bir kısmına işaret ediyorsa işe yarar. `status = 'active'` koşulu satırların %80'ini getiriyorsa planlayıcı index'i görmezden gelip sequential scan yapar — ve haklıdır, çünkü rastgele sayfa okumak sıralı okumaktan pahalıdır. Cinsiyet, boolean bayrak gibi düşük kardinaliteli kolonlara tek başına index atmak neredeyse hep israftır.
...
Çok kolonlu index'lerde PostgreSQL "en sol önek" (leftmost prefix) kuralıyla çalışır. `(tenant_id, created_at)` index'i `tenant_id` üzerinden yapılan aramaya da hizmet eder, ancak yalnızca `created_at` filtreleyen bir sorguya verimli biçimde yardım etmez. Genel yaklaşım: eşitlik karşılaştırılan kolonlar başa, aralık (`>`, `BETWEEN`) ve `ORDER BY` kolonları sona. Böylece index hem filtrelemeyi hem sıralamayı tek geçişte karşılar ve planda maliyetli bir sort adımı belirmez.

## Index tipini veri belirler

B-tree varsayılandır ve karşılaştırma operatörleriyle çalışan her şeyi kapsar. Fakat:
